...
# Окно, за которое собираются соседи по сегменту (район × размерная категория).
# Снимок самого объекта может быть старше этого окна — поэтому дата объекта
# возвращается отдельным полем as_of, а не подразумевается.
SEGMENT_WINDOW_DAYS: int = 14
...
SCHEDULER_REINDEX_TIMEOUT_S: int = 900
