...
class ForecastExplanation(BaseModel):
    ...
    recommendation: str = Field(description="Практическая рекомендация для отельеров или туристов")
...
{format_instructions}""")
    ...
    def _generate_recommendation(self, occ: float | None, factors: list[dict]) -> str:
        ...
        if occ > 70:
            return "Высокий спрос: рекомендуется повысить цены на 10-15% и бронировать заранее."
        if occ > 50:
            return "Умеренный спрос: стандартные цены, хорошее время для бронирования."
        if occ > 30:
            return "Низкий спрос: рекомендуются скидки и спецпредложения."
        return "Очень низкий спрос: значительные скидки или пакетные предложения."
